...
export class PredictionEngine {
    ...
    predictFlatSprint(profile, distance, athlete) {
        ...
        return { time: finalTime.toFixed(2), range: 0.10, splits, tags: [usedMethod], sources: [usedMethod] };
    }
     ...
     * PRESERVES all 11 technical analysis points.
     */
    generateAdvice(profile, metrics, athlete) {
        ...
        // 1. Ratio Block/Fly
        if (metrics.test_30m_block && metrics.test_30m_fly) {
            const ratio = metrics.test_30m_block / metrics.test_30m_fly; 
            if (ratio > 1.48) { weaknesses.push("Déficit d'accélération."); advice.push("Priorité: Force maximale (Squat) et puissance de poussée."); }
            else if (ratio < 1.30) { strengths.push("Excellent accélérateur."); advice.push("Priorité: Maintien de vitesse et mécanique de sprint lancé."); }
        }
        ...
        if (metrics.contact_time_r || metrics.contact_time_l) {
            ...
            const ct = val > 10 ? val : val * 1000;
            const targets = CONTACT_TIME_TARGETS[athlete.gender || 'M'];
            let match = targets.find(t => ct >= t.min && ct <= t.max);
            if (match) advice.push(`Temps de contact : ${match.label}. ${match.obs}`);
            else if (ct < 85) strengths.push("Stiffness World Class !");
            else { weaknesses.push("Temps de contact longs."); advice.push("Travaillez la raideur (Pliométrie, Corde à sauter)."); }
        }

        // 3. Elasticity (SSC)
        if (metrics.cmj_height && metrics.sj_height) {
            const index = ((metrics.cmj_height - metrics.sj_height) / metrics.sj_height) * 100;
            if (index < 8) advice.push("Utilisation inefficace du cycle étirement-détente (pliométrie requise).");
            else if (index > 15) strengths.push("Excellente réactivité élastique.");
        }

        // 4. Asymmetry Step Length
        if (metrics.step_len_avg_r && metrics.step_len_avg_l) {
            if (Math.abs(metrics.step_len_avg_r - metrics.step_len_avg_l) > 0.08) weaknesses.push("Forte asymétrie de foulée. Risque de blessure.");
        }

        // 5. Frequency vs Amplitude
        if (metrics.step_len_avg_r && metrics.test_30m_fly) {
            const freq = (30/metrics.test_30m_fly) / metrics.step_len_avg_r;
            if (freq < 3.9) advice.push("Fréquence basse: travaillez la cadence et la vélocité gestuelle.");
            else if (freq > 4.9) advice.push("Amplitude réduite: travaillez la force de poussée.");
        }

        // 6. Start Geometry
        if (metrics.first_contact_dist && metrics.first_contact_dist < 0.50) advice.push("1er appui trop court: cherchez plus de projection vers l'avant.");

        // 7. Power/Weight Ratio
        if (metrics.cmj_power && metrics.cmj_power < 45) weaknesses.push("Puissance relative (W/kg) insuffisante.");

        // 8. Horizontal Force (Bounds)
        if (metrics.bound_test && metrics.bound_test < 13) advice.push("Travaillez les bonds pour améliorer la force horizontale.");

        // 9. Speed Endurance
        if (this.calculateFatigueIndex(metrics, profile.vmax) > 1.28) advice.push("Chute de vitesse après 60m: priorisez l'endurance de vitesse spécifique.");

        // 10. Start Latency
        if (metrics.time_to_first_contact && metrics.time_to_first_contact > 0.40) advice.push("Temps de sortie de blocks trop long.");
        
        // 11. Youth Specific
        if (athlete.age < 20 && metrics.pb_100m && profile.vmax > 10.5) advice.push("Vitesse exceptionnelle pour l'âge. Focus sur la musculation posturale.");
